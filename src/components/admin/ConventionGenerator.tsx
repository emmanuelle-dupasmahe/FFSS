...
import { FileText, Printer, Users, Activity, Eye, Edit3, Signature } from "lucide-react";
...
export default function ConventionGenerator({ demandeDPS, calculRIS }: { demandeDPS: any, calculRIS: any }) {
    ...
    const [typeConvention, setTypeConvention] = useState<'PUBLIC' | 'ACTEUR'>('PUBLIC');
    const [inclureSignature, setInclureSignature] = useState(true);

    const dateDeb = demandeDPS.eventDate ? new Date(demandeDPS.eventDate).toLocaleDateString('fr-FR') : "";
    const dateFin = demandeDPS.endDate ? new Date(demandeDPS.endDate).toLocaleDateString('fr-FR') : "";
    ...
    const [infos, setInfos] = useState({
        ...
        texteArt46: "Les intervenants sont revêtus de leur tenue officielle. Le chef de poste prendra contact avec le bénéficiaire dès son arrivée sur site pour vérifier la concordance avec les clauses techniques. Les intervenants et véhicules sont dotés de moyens radio sur fréquence propre.",
        ...
        texteRepas: "Les repas et les boissons des secouristes présents seront pris en charge par l'organisateur.",
        texteArt53: "L'intervention des secouristes demeure bénévole (but non lucratif). Toutefois, l'organisateur dédommage l'association des frais engendrés, pour un montant défini dans le devis remis par l'association. Cette somme sera réglée par virement ou par chèque libellé à l'ordre de l'Association.",
    ...
    const page1 = (
        <>
            ...
            <textarea
                rows={4}
                value={infos.texteIntro}
                onChange={e => setInfos({ ...infos, texteIntro: e.target.value })}
                className={inlineTextareaStyle + " mb-6 indent-8"}
            />

            <h3 className="font-bold uppercase text-[13px] bg-gray-100 text-black p-1 mb-2">1. Association Prestataire :</h3>
            <textarea
                rows={5}
                value={infos.texteArt1}
                onChange={e => setInfos({ ...infos, texteArt1: e.target.value })}
            ...
            <textarea
                rows={3}
                value={infos.texteArt31}
                onChange={e => setInfos({ ...infos, texteArt31: e.target.value })}
                className={inlineTextareaStyle + " mb-2"}
            />
            ...
            {typeConvention === 'PUBLIC' ? (
                ...
                <div className="mb-4 font-bold bg-yellow-50 p-2 border border-yellow-200 text-black text-[12px] space-y-2">
            ...
            <ul className="mb-4 list-disc ml-6 text-[12px] text-slate-900">
                ...
                <li>Date et heures : Du {infos.dateDebut} ({infos.heureDebut}) au {infos.dateFin} ({infos.heureFin})</li>
            ...
            <textarea
                rows={3}
                value={infos.texteArt35}
                onChange={e => setInfos({ ...infos, texteArt35: e.target.value })}
                className={inlineTextareaStyle + " mb-2"}
            />
        </>
    );
    ...
    const page2 = (
        <>
            ...
            <textarea
                rows={4}
                value={infos.texteArt41}
                onChange={e => setInfos({ ...infos, texteArt41: e.target.value })}
                className={inlineTextareaStyle + " mb-4"}
            />
            ...
            <textarea
                rows={3}
                value={infos.texteArt431}
                onChange={e => setInfos({ ...infos, texteArt431: e.target.value })}
                className={inlineTextareaStyle + " mb-2"}
            />
            <p className="mb-1 italic text-black font-semibold">4.3.2 Moyens matériels :</p>
            <textarea
                rows={1}
                value={infos.texteArt432}
                onChange={e => setInfos({ ...infos, texteArt432: e.target.value })}
                className={inlineTextareaStyle + " mb-4"}
            />
            ...
            <textarea
                rows={7}
                value={infos.texteArt44}
                onChange={e => setInfos({ ...infos, texteArt44: e.target.value })}
                className={inlineTextareaStyle + " mb-4 font-mono text-[11px] bg-slate-50/50"}
            />

            <p className="font-bold underline text-[11px] mb-1 text-black">4.5 Transport des victimes :</p>
            <textarea
                rows={2}
                value={infos.texteArt45}
                onChange={e => setInfos({ ...infos, texteArt45: e.target.value })}
                className={inlineTextareaStyle + " mb-4"}
            />
            ...
            <textarea
                rows={3}
                value={infos.texteArt46}
                onChange={e => setInfos({ ...infos, texteArt46: e.target.value })}
                className={inlineTextareaStyle + " mb-2"}
            />
        </>
    );
    ...
    const page3 = (
        <>
            ...
            <textarea
                rows={2}
                value={infos.texteArt51}
                onChange={e => setInfos({ ...infos, texteArt51: e.target.value })}
                className={inlineTextareaStyle + " mb-2 font-mono text-[11px]"}
            />

            <p className="mb-1 text-emerald-700 font-bold text-[11px] px-1">Prise en charge restauration / boissons :</p>
            <textarea
                rows={2}
                value={infos.texteRepas}
                onChange={e => setInfos({ ...infos, texteRepas: e.target.value })}
                className={inlineTextareaStyle + " mb-2 border-l-2 border-emerald-500 pl-2 font-medium bg-emerald-50/20 text-emerald-950"}
            />

            <p className="mb-2 text-[12px] text-slate-900"><strong>5.2 Modalités opérationnelles :</strong> Correspondant : M. {infos.contactNom} (Tél: {infos.contactTel}). Le commandement du dispositif sera assuré par l'association prestataire.</p>

            <p className="font-bold underline text-[11px] mb-1 text-black">5.3 Modalités financières :</p>
            <textarea
                rows={3}
                value={infos.texteArt53}
                onChange={e => setInfos({ ...infos, texteArt53: e.target.value })}
                className={inlineTextareaStyle + " mb-6"}
            />

            <h3 className="font-bold uppercase text-[13px] bg-gray-100 text-black p-1 mb-2">9. Litiges :</h3>
            <textarea
                rows={3}
                value={infos.texteArt9}
                onChange={e => setInfos({ ...infos, texteArt9: e.target.value })}
                className={inlineTextareaStyle + " mb-6"}
            />
                    ...
                    {inclureSignature && (
                        <div className="relative h-24 mt-2">
                            <img
                                src="/cachet-asso.png"
                                alt="Cachet ASSTSF"
                                className="absolute top-0 left-0 w-48 h-48 object-contain opacity-80"
                            />
                            <img
                                src="/pres.png"
                                alt="Signature Président"
                                className="absolute top-4 left-12 w-32 h-16 object-contain mix-blend-multiply"
                            />
                        </div>
                    )}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-1 p-6 bg-white dark:bg-white/5 rounded-3xl border border-slate-200 dark:border-white/10 space-y-6 relative lg:sticky top-8 overflow-y-auto max-h-[calc(100vh-4rem)] z-10">
                ...
                <div className="flex items-center space-x-2 p-3 bg-slate-50 dark:bg-[#001A3D] rounded-xl border border-slate-200 dark:border-white/10">
                    <Checkbox
                        id="sign"
                        checked={inclureSignature}
                        onCheckedChange={(v: boolean) => setInclureSignature(v)}
                    />
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        <Button
                            ...
                            onClick={() => setTypeConvention('PUBLIC')}
                        ...
                        <Button
                            ...
                            onClick={() => setTypeConvention('ACTEUR')}
                            className="flex-1 rounded-xl text-[10px] font-black uppercase h-10"
                        >
                ...
                <div className="space-y-4 border-t border-slate-200 dark:border-white/10 pt-4">
                    <div className="flex items-center gap-2 text-blue-600">
                        <Edit3 size={16} />
                        <p className="text-[10px] font-black uppercase">Variables Fixes</p>
                    </div>
            ...
            {/* APERÇU ÉCRAN INTERACTIF DEVENU "MULTI-PAGES" (Droite) */}
            ...
            {/* LA VRAIE ZONE PDF CACHÉE : On rend les 3 pages exactement au format A4 */}
