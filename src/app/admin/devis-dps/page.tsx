    ...
    return (
        <div className="p-2 md:p-4 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {demandes.map((d) => {
                    // 🎨 Gestion dynamique des couleurs des badges selon le statut
                    let badgeStyle = "bg-amber-500/10 text-amber-500 border-amber-500/20";
                    if (d.status === "TRAITE" || d.status === "VALIDE") {
                        badgeStyle = "bg-blue-500/10 text-blue-400 border-blue-500/20";
                    }
                    if (d.isSigned || d.status === "SIGNE") {
                        ...
                        >
                            ...
                            <div className="p-5 border-b border-white/5 bg-white/5">
                                <div className="flex justify-between items-start mb-2">
                                    <Badge
                                        className={`uppercase text-[9px] font-black tracking-widest px-2 py-1 ${badgeStyle}`}
                                        variant="outline"
                                    >
                                        {d.isSigned ? "SIGNE" : d.status.replace("_", " ")}
                                    </Badge>
